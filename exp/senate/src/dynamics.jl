...
function base_dynamics(x::BlockVector, u::BlockVector, w::BlockVector, player_idx::Union{Int,Nothing}=nothing; config::PlayerConfig)
    BlockVector(mapreduce(vcat, enumerate(zip(x.blocks, w.blocks))) do (i, (x_i, m))
            ...
            x_i + config.dt * ([x_move; y_move] + m) # TODO incr to see diff in ekf 
        end, length.(x.blocks))
end
...
function attraction_dynamics_model(x_all_senators::BlockVector, u::BlockVector, ms::BlockVector; config::PlayerConfig)
    BlockVector(mapreduce(vcat, enumerate(zip(x_all_senators.blocks, ms.blocks))) do (i, (x_i, m))
            ...
            for j in 1:config.num_senators
                ...
                if is_dual
                    debug_nan_inf(x_j, "x_j", i, j)
                end

                diff = x_j - x_i
                if is_dual
                    debug_nan_inf(diff, "diff", i, j)
                end

                dist_sq = dot(diff, diff)
                if is_dual
                    debug_nan_inf(dist_sq, "dist_sq", i, j)
                end
                ...
                if is_dual
                    debug_nan_inf(strength_factor, "strength_factor", i, j)
                end

                force_update = config.attraction_matrix[i, j] * strength_factor .* sign.(diff) .* sqrt.(abs.(diff))
                if is_dual
                    debug_nan_inf(force_update, "force_update", i, j)
                end
